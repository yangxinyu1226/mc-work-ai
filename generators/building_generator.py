...
class BuildingGenerator:
    ...
    def generate(self, description: str) -> dict:
        ...
        system_prompt = """你是一个Minecraft建筑设计师。你的任务是根据用户的描述，设计一个Minecraft建筑，并输出一个JSON数组，其中每个元素都是一个几何图元生成任务。每个任务都必须包含'tool'和'args'字段。

可用的几何图元工具及其参数示例：
- 'cube': {'x': 0, 'y': 0, 'z': 0, 'size_x': 5, 'size_y': 5, 'size_z': 5, 'block_type': 'minecraft:stone', 'hollow': false}
- 'hollow_cube': {'x': 0, 'y': 0, 'z': 0, 'size_x': 5, 'size_y': 5, 'size_z': 5, 'block_type': 'minecraft:oak_planks'} (用于空心结构，如房屋主体)
- 'line': {'x1': 0, 'y1': 0, 'z1': 0, 'x2': 10, 'y2': 0, 'z2': 0, 'block_type': 'minecraft:oak_planks'}
- 'sphere': {'x': 0, 'y': 0, 'z': 0, 'radius': 5, 'block_type': 'minecraft:glass', 'hollow': true}
- 'cylinder': {'x': 0, 'y': 0, 'z': 0, 'radius': 5, 'height': 10, 'block_type': 'minecraft:cobblestone', 'hollow': false}
- 'pyramid': {'x': 0, 'y': 0, 'z': 0, 'base_size': 7, 'block_type': 'minecraft:sandstone'}
- 'circle': {'x': 0, 'y': 0, 'z': 0, 'radius': 5, 'block_type': 'minecraft:dirt', 'hollow': false} (平面圆)
...
2.  'block_type'必须是有效的Minecraft方块ID，例如'minecraft:stone'、'minecraft:oak_planks'。
3.  **对于房屋、塔楼等可进入的建筑主体，请务必使用'hollow_cube'工具来创建空心结构。**
...
[
  {
    "tool": "hollow_cube",
    "args": {"x": 0, "y": 0, "z": 0, "size_x": 8, "size_y": 6, "size_z": 8, "block_type": "minecraft:oak_planks"}
  },
  {
    "tool": "cube",
    "args": {"x": 3, "y": 1, "z": 0, "size_x": 2, "size_y": 3, "size_z": 1, "block_type": "minecraft:air"} 
  }, 
  {
    "tool": "single_block",
    "args": {"x": 3, "y": 1, "z": 0, "block_type": "minecraft:oak_door"}
  },
  {
    "tool": "single_block",
    "args": {"x": 4, "y": 1, "z": 0, "block_type": "minecraft:oak_door"}
  },
  {
    "tool": "cube",
    "args": {"x": 1, "y": 2, "z": 0, "size_x": 2, "size_y": 2, "size_z": 1, "block_type": "minecraft:glass_pane"}
  }
]
```
"""    
